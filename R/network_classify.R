...
network_classify <- function(L='label',data_train,data_test,nf,p,corr,f_type,s,nc,kernel){
  ...
  if(f_type<4){
    ...
    model1 <- svm(newdata$new_train,newdata$train_label,type="C-classification",class.weights = wts,kernel=kernel)
    ...
    wts <- nrow(newdata$new_train) / table(newdata$train_label)
    model1 <- svm(Data_train,newdata$train_label,type="C-classification",class.weights = wts,kernel=kernel)
